...
public final class MPlaceMapperFactory implements ObjectMapperFactory<test.model.emap.mixedkeys.MPlaceMapper,Place> {
	...
	@Override
	public Class<test.model.emap.mixedkeys.MPlaceMapper> getMapperType() {
		return test.model.emap.mixedkeys.MPlaceMapper.class;
	}

	@Override
	public test.model.emap.mixedkeys.MPlaceMapper createMapper(JavaSession session) {
		return new MPlaceMapperImpl(session);
	}

	final static class MPlaceMapperImpl implements MPlaceMapper, at.bestsolution.persistence.java.JavaObjectMapper<Place>,at.bestsolution.persistence.java.RefreshableObjectMapper<Place>, ResolveDelegate {
		private final JavaSession session;
		private boolean inAutoResolve;
		private static final Logger LOGGER = Logger.getLogger(MPlaceMapperImpl.class);
		private static MetaData META_DATA;

		public MPlaceMapperImpl(JavaSession session) {
		...
		// by JavaObjectMapperGenerator
		@Override
		public <K extends at.bestsolution.persistence.Key<Place>> long selectVersion(K id) {
			...
			try {
				try {
					if( session.getDatabaseSupport().isDefaultLowerCase() ) {
						pStmt = connection.prepareStatement("SELECT " + getLockColumn() + " FROM \"mplace\" WHERE \"(\"id\" = ?)");
					} else {
						pStmt = connection.prepareStatement("SELECT " + getLockColumn() + " FROM \"MPLACE\" WHERE \"(\"ID\" = ?)");
					}
		...
		public final Place map_default_Place(Connection connection, ResultSet set) throws SQLException {
			...
			final EClass eClass = test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace();
		...
		private final void map_default_Place_complete_refresh(Place rv, Connection connection, ResultSet set, Set<Object> refreshedObjects) throws SQLException {
			...
			((LazyEObject)rv).setProxyDelegate(this);
			{
				...
				if( ((LazyEObject)rv).isResolved(r) ) {
					test.model.emap.mixedkeys.MPersonMapper m = session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class);
					RefreshableObjectMapper<test.model.mixedkeys.Person> mr = (RefreshableObjectMapper<test.model.mixedkeys.Person>)m;
					List<test.model.mixedkeys.Person> list = m.selectAllForPlace((long)getPrimaryKey(rv).getValue("id"));
					Util.syncLists(rv.getLikedBy(), list);
					for( test.model.mixedkeys.Person e : rv.getLikedBy() ) {
			...
			private List<Key> extractObjectIds(Place... object) {
			...
			private final void clearManyToManyById_LikedBy(Connection connection, List<Key> objectIds) throws SQLException {
				...
				String sql;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sql = "DELETE FROM \"mlikes_person_place\" WHERE \"fk_place\" IN ("+b.toString()+")";
				} else {
					sql = "DELETE FROM \"MLIKES_PERSON_PLACE\" WHERE \"FK_PLACE\" IN ("+b.toString()+")";
				}
			...
			private final RelationSQL createDeleteManyToManyRelationSQL_likedBy(final JavaSession session, final Connection c, final Place self, final test.model.mixedkeys.Person opposite) {
				final String sql;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sql = "DELETE FROM \"mlikes_person_place\" WHERE \"fk_place\" = ? AND \"fk_person\" = ?";
				} else {
					sql = "DELETE FROM \"MLIKES_PERSON_PLACE\" WHERE \"FK_PLACE\" = ? AND \"FK_PERSON\" = ?";
				}
					return new RelationSQL() {
						...
						public void execute() throws PersistanceException {
							...
							final test.model.emap.mixedkeys.MPlaceMapper selfMapper = session.createMapper(test.model.emap.mixedkeys.MPlaceMapper.class);
							final test.model.emap.mixedkeys.MPersonMapper oppositeMapper = session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class);
							final test.model.emap.mixedkeys.MPlaceMapper.Key selfId = session.getPrimaryKey(selfMapper, self);
							final test.model.emap.mixedkeys.MPersonMapper.Key oppositeId = session.getPrimaryKey(oppositeMapper, opposite);
							if( isDebug ) {
							...
							try {
								...
								pstmt.setString(2, oppositeId.id());
								pstmt.execute();
								if( isDebug ) {
			...
			private final RelationSQL createInsertManyToManyRelationSQL_likedBy(final JavaSession session, final Connection c, final Place self, final test.model.mixedkeys.Person opposite) {
				final String sql;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sql = "INSERT INTO \"mlikes_person_place\" (\"fk_place\",\"fk_person\") VALUES (?,?)";
				} else {
					sql = "INSERT INTO \"MLIKES_PERSON_PLACE\" (\"FK_PLACE\",\"FK_PERSON\") VALUES (?,?)";
				}
					return new RelationSQL() {
			...
			private final RelationSQL createClearManyToManyRelationSQL_likedBy(final JavaSession session, final Connection c, final Place self) {
				final String sql;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sql = "DELETE FROM \"mlikes_person_place\" WHERE \"fk_place\" = ?";
				} else {
					sql = "DELETE FROM \"MLIKES_PERSON_PLACE\" WHERE \"FK_PLACE\" = ?";
				}
		...
		// by QueryGenerator
		@Override
		public final test.model.mixedkeys.Place selectById(final long id) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Check id cache for object");
			{
				...
				final Place rv = session.getCache().getObject(eClass, key);
				if( rv != null ) {
			...
			String query;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					query = Util.loadFile(getClass(), "MPlace_selectById_default_lc.sql");
				} else {
					query = Util.loadFile(getClass(), "MPlace_selectById_default.sql");
				}
		...
		// by QueryGenerator
		@Override
		public final java.util.List<test.model.mixedkeys.Place> selectAllForPerson(final String personId) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Executing selectAllForPerson");
		
			String query;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					query = Util.loadFile(getClass(), "MPlace_selectAllForPerson_default_lc.sql");
				} else {
					query = Util.loadFile(getClass(), "MPlace_selectAllForPerson_default.sql");
				}
			...
			for(int i = 0; i < processedSQL.dynamicParameterNames.size(); i++) {
				if( "personId".equals(processedSQL.dynamicParameterNames.get(i)) ) {
					if( isDebug ) {
						...
						pStmt.setString(paramIndex++,personId);
		...
		public final void update(final Place object) {
			...
			// Built the query
			at.bestsolution.persistence.java.DatabaseSupport.UpdateStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"MPLACE").createUpdateStatement(PKLayout, getLockColumn());
			// NEW:
			// simple direct mapped attributes
			// * name
			stmt.addString("NAME", (java.lang.String)session.getTransactionAttribute(object,test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace_Name()));
			...
			try {
				...
				// * likedBy
				{
					...
					for (JavaSession.ChangeDescription change : changes) {
						if (change.getFeature() == test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace_LikedBy()) {
							delta = change;
							break;
						}
					}
					...
					if (delta != null) {
						...
						for (Object addition : delta.getAdditions()) {
							final test.model.emap.mixedkeys.MPersonMapper.Key oppositePK = oppositeMapper.getPrimaryKey((test.model.mixedkeys.Person)addition);
							//final Object oppositePK = oppositeMapper.getPrimaryKeyValue((test.model.mixedkeys.Person)addition);
							// TODO test for new object?
							session.scheduleRelationSQL(createInsertManyToManyRelationSQL_likedBy(session, connection, object, (test.model.mixedkeys.Person)addition));
						}
		
						for (Object removal : delta.getRemovals()) {
							final test.model.emap.mixedkeys.MPersonMapper.Key oppositePK = oppositeMapper.getPrimaryKey((test.model.mixedkeys.Person)removal);
							//final Object oppositePK = oppositeMapper.getPrimaryKeyValue((test.model.mixedkeys.Person)removal);
							// TODO test for new object?
							session.scheduleRelationSQL(createDeleteManyToManyRelationSQL_likedBy(session, connection, object, (test.model.mixedkeys.Person)removal));
						}
		...
		public final void insert(final Place object) {
			...
			if( "Firebird".equals(session.getDatabaseType()) ) {
				sequenceExpressions.put("ID","NEXT VALUE FOR SEQ_MPLACE_ID");
			}
			if( "Postgres".equals(session.getDatabaseType()) ) {
			}
			// Build the SQL
			at.bestsolution.persistence.java.DatabaseSupport.InsertStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"MPLACE").createInsertStatement(PKLayout, sequenceExpressions, getLockColumn());
		
		
			// handle simple direct mapped attributes
			// * name
			{
				Object o = session.getTransactionAttribute(object,test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace_Name());
				if( o != null ) {
			...
			try {
				...
				// likedBy
				for(test.model.mixedkeys.Person e : object.getLikedBy()) {
		...
		final int deleteAll(InternalQueryCriteria criteria) {
			...
			session.preExecuteDeleteMany(test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace());
		
			// build delete query
			String deleteQuery;
			if( session.getDatabaseSupport().isDefaultLowerCase() ) {
				deleteQuery = "DELETE FROM \"mplace\"";
			} else {
				deleteQuery = "DELETE FROM \"MPLACE\"";
			}
			...
			try {
				// build select query
				String selectQuery = "SELECT ID FROM \"MPLACE\"";
		...
		public final void deleteAll() {
			...
			session.scheduleAfterTransaction(new at.bestsolution.persistence.java.UnregisterAllObjectsAfterTx(test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace()));
			...
			try {
				...
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					objectIdSQL = "SELECT \"id\" FROM \"mplace\"";
				} else {
					objectIdSQL = "SELECT \"ID\" FROM \"MPLACE\"";
				}
		...
		// by JavaInsertUpdateGenerator
		@Override
		public <K extends at.bestsolution.persistence.Key<Place>> void deleteById(K... keys) {
			final List<Key> keyz= new ArrayList<Key>();
			for (at.bestsolution.persistence.Key<Place> key : keys) {
		...
		public final void deleteById(List<Key> objectIds) {
			...
			try {
				...
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sqlBuilder.append("DELETE FROM \"mplace\" WHERE \"id\" IN (");
				} else {
					sqlBuilder.append("DELETE FROM \"MPLACE\" WHERE \"ID\" IN (");
				}
				...
				if (!objectIds.isEmpty()) {
					...
					try {
						...
						while (stmtParamIt.hasNext()) {
							...
							Util.setKeyValue(stmt, sqlobjectIdsIdx, test.model.emap.mixedkeys.MPlaceMapper.PKLayout, obj, "id");
							sqlobjectIdsIdx++;
						}
						stmt.execute();
					}
		...
		public final void delete(Place... object) {
			...
			List<at.bestsolution.persistence.Key<Place>> keyList = new ArrayList<>();
			for (Place o : object) {
		...
		final boolean doResolve(LazyEObject eo, Object proxyData, EStructuralFeature f) {
			...
			if(eo instanceof Place) {
				test.model.mixedkeys.Place target = (test.model.mixedkeys.Place)eo;
				switch(f.getFeatureID()) {
				  // likedBy
				  case test.model.mixedkeys.MixedkeysPackage.PLACE__LIKED_BY: {
				    target.getLikedBy().addAll(session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class).selectAllForPlace((long)getPrimaryKey(target).getValue("id")));
				    return true;
				  }
				  default : {
				    return false;
		...
		static {
			...
			REFERENCE_MAPPER.put("likedBy",test.model.emap.mixedkeys.MPersonMapper.class);
		}

		// by JavaObjectMapperGenerator
		public EClass getEClass() {
			return test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPlace();
		}
		...
		// by JavaObjectMapperGenerator
		public final Key getPrimaryKey(Place o) {
		...
		// by JavaObjectMapperGenerator
		protected final Key getPrimaryKeyForTx(Place o) {
		...
		// by JavaObjectMapperGenerator
		private Map<String, Object> parseFKValuesForLikedBy(ResultSet set) throws SQLException {
	...
	// by JavaObjectMapperGenerator
	public final NamedQuery<test.model.mixedkeys.Place> createNamedQuery(final JavaSession session, String name) {
			if( "selectById".equals(name) ) {
				return new NamedQuery<test.model.mixedkeys.Place>() {
					public final List<test.model.mixedkeys.Place> queryForList(Object... parameters) {
						throw new UnsupportedOperationException("This is a single value query");
					}

					public final test.model.mixedkeys.Place queryForOne(Object... parameters) {
			...
			if( "selectAllForPerson".equals(name) ) {
				return new NamedQuery<test.model.mixedkeys.Place>() {
					public final List<test.model.mixedkeys.Place> queryForList(Object... parameters) {
						return createMapper(session).selectAllForPerson((String)parameters[0]);
					}
