...
public final class MAddressMapperFactory implements ObjectMapperFactory<test.model.emap.mixedkeys.MAddressMapper,Address> {
	...
	@Override
	public Class<test.model.emap.mixedkeys.MAddressMapper> getMapperType() {
		return test.model.emap.mixedkeys.MAddressMapper.class;
	}

	@Override
	public test.model.emap.mixedkeys.MAddressMapper createMapper(JavaSession session) {
		return new MAddressMapperImpl(session);
	}

	final static class MAddressMapperImpl implements MAddressMapper, at.bestsolution.persistence.java.JavaObjectMapper<Address>,at.bestsolution.persistence.java.RefreshableObjectMapper<Address>, ResolveDelegate {
		private final JavaSession session;
		private boolean inAutoResolve;
		private static final Logger LOGGER = Logger.getLogger(MAddressMapperImpl.class);
		private static MetaData META_DATA;

		public MAddressMapperImpl(JavaSession session) {
			...
			try {
				try {
					if( session.getDatabaseSupport().isDefaultLowerCase() ) {
						pStmt = connection.prepareStatement("SELECT " + getLockColumn() + " FROM \"maddress\" WHERE \"(\"id\" = ?)");
					} else {
						pStmt = connection.prepareStatement("SELECT " + getLockColumn() + " FROM \"MADDRESS\" WHERE \"(\"ID\" = ?)");
					}
		...
		public final Address map_default_Address(Connection connection, ResultSet set) throws SQLException {
			...
			final EClass eClass = test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress();
		...
		private final void map_default_Address_complete_refresh(Address rv, Connection connection, ResultSet set, Set<Object> refreshedObjects) throws SQLException {
			...
			((LazyEObject)rv).setProxyDelegate(this);
			{
				...
				if( ((LazyEObject)rv).isResolved(r) ) {
						test.model.mixedkeys.Person v = rv.getPerson();
						RefreshableObjectMapper<test.model.mixedkeys.Person> mr = (RefreshableObjectMapper<test.model.mixedkeys.Person>)session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class);
						if( v != null && ! refreshedObjects.contains(v) ) {
							mr.refreshWithReferences(v,refreshedObjects);
						} else {
							at.bestsolution.persistence.Key<test.model.mixedkeys.Person> currentId = v == null ? null : mr.getPrimaryKey(v);
							if( proxy.person.equals(currentId) ) {
								EClass eClass = test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPerson();
			...
			{
				EObject eo = (EObject)rv;
				EReference r = (EReference)eo.eClass().getEStructuralFeature("tags");
				if( ((LazyEObject)rv).isResolved(r) ) {
					test.model.emap.mixedkeys.MTagMapper m = session.createMapper(test.model.emap.mixedkeys.MTagMapper.class);
					RefreshableObjectMapper<test.model.mixedkeys.Tag> mr = (RefreshableObjectMapper<test.model.mixedkeys.Tag>)m;
					List<test.model.mixedkeys.Tag> list = m.selectAllForAddress((java.math.BigInteger)getPrimaryKey(rv).getValue("id"));
					Util.syncLists(rv.getTags(), list);
					for( test.model.mixedkeys.Tag e : rv.getTags() ) {
		...
		// by QueryGenerator
		@Override
		public final test.model.mixedkeys.Address selectById(final java.math.BigInteger id) {
			...
			String query;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					query = Util.loadFile(getClass(), "MAddress_selectById_default_lc.sql");
				} else {
					query = Util.loadFile(getClass(), "MAddress_selectById_default.sql");
				}
		...
		// by QueryGenerator
		@Override
		public final java.util.List<test.model.mixedkeys.Address> selectAllForPerson(final String personId) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Executing selectAllForPerson");
		
			String query;
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					query = Util.loadFile(getClass(), "MAddress_selectAllForPerson_default_lc.sql");
				} else {
					query = Util.loadFile(getClass(), "MAddress_selectAllForPerson_default.sql");
				}
		...
		public final void update(final Address object) {
			...
			// Built the query
			at.bestsolution.persistence.java.DatabaseSupport.UpdateStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"MADDRESS").createUpdateStatement(PKLayout, getLockColumn());
			// NEW:
			// simple direct mapped attributes
			// * street
			stmt.addString("STREET", (java.lang.String)session.getTransactionAttribute(object,test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress_Street()));
			// one to one references
			// * person (Person)
			if( object.getPerson() != null ) {
				final test.model.emap.mixedkeys.MPersonMapper refMapper = session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class);
				final test.model.emap.mixedkeys.MPersonMapper.Key refKey = session.getPrimaryKey(refMapper, object.getPerson());
		...
		public final void insert(final Address object) {
			...
			// Build the SQL
			at.bestsolution.persistence.java.DatabaseSupport.InsertStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"MADDRESS").createInsertStatement(PKLayout, sequenceExpressions, getLockColumn());
		
		
			// handle simple direct mapped attributes
			// * street
			{
				Object o = session.getTransactionAttribute(object,test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress_Street());
				if( o != null ) {
		...
		final int deleteAll(InternalQueryCriteria criteria) {
			...
			session.preExecuteDeleteMany(test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress());
		
			// build delete query
			String deleteQuery;
			if( session.getDatabaseSupport().isDefaultLowerCase() ) {
				deleteQuery = "DELETE FROM \"maddress\"";
			} else {
				deleteQuery = "DELETE FROM \"MADDRESS\"";
			}
		...
		public final void deleteAll() {
			...
			session.scheduleAfterTransaction(new at.bestsolution.persistence.java.UnregisterAllObjectsAfterTx(test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress()));
			...
			try {
				...
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					objectIdSQL = "SELECT \"id\" FROM \"maddress\"";
				} else {
					objectIdSQL = "SELECT \"ID\" FROM \"MADDRESS\"";
				}
		...
		public final void deleteById(List<Key> objectIds) {
			...
			try {
				...
				if( session.getDatabaseSupport().isDefaultLowerCase() ) {
					sqlBuilder.append("DELETE FROM \"maddress\" WHERE \"id\" IN (");
				} else {
					sqlBuilder.append("DELETE FROM \"MADDRESS\" WHERE \"ID\" IN (");
				}
				...
				if (!objectIds.isEmpty()) {
					...
					try {
						...
						while (stmtParamIt.hasNext()) {
							...
							Util.setKeyValue(stmt, sqlobjectIdsIdx, test.model.emap.mixedkeys.MAddressMapper.PKLayout, obj, "id");
							sqlobjectIdsIdx++;
						}
						stmt.execute();
					}
		...
		final boolean doResolve(LazyEObject eo, Object proxyData, EStructuralFeature f) {
			...
			if(eo instanceof Address) {
				test.model.mixedkeys.Address target = (test.model.mixedkeys.Address)eo;
				switch(f.getFeatureID()) {
				  // person
				  case test.model.mixedkeys.MixedkeysPackage.ADDRESS__PERSON: {
				    {
				      
				      EClass eClass = test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPerson();
				      test.model.mixedkeys.Person o = session.getCache().getObject(eClass, ((ProxyData_Address)proxyData).person);
				      if( o == null ) {
				        o = session.createMapper(test.model.emap.mixedkeys.MPersonMapper.class).selectById(((ProxyData_Address)proxyData).person.id());
				      } else {
				        if( LOGGER.isDebugEnabled() ) {
				          LOGGER.debug("Using cached version");
				        }
				  ...
				  // tags
				  case test.model.mixedkeys.MixedkeysPackage.ADDRESS__TAGS: {
				    target.getTags().addAll(session.createMapper(test.model.emap.mixedkeys.MTagMapper.class).selectAllForAddress((java.math.BigInteger)getPrimaryKey(target).getValue("id")));
				    return true;
				  }
				  default : {
				    return false;
		...
		static {
			...
			TYPE_MAPPING.put("person",JDBCType.STRING);
			REF_ID_FEATURES.put("person",test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getPerson_Id());
			REFERENCE_MAPPER.put("person",test.model.emap.mixedkeys.MPersonMapper.class);
			REFERENCE_MAPPER.put("tags",test.model.emap.mixedkeys.MTagMapper.class);
			REFERENCE_FEATURES.add(test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress_Person());
		}

		// by JavaObjectMapperGenerator
		public EClass getEClass() {
			return test.model.mixedkeys.MixedkeysPackage.eINSTANCE.getAddress();
		}
		...
		private Map<String, Object> parseFKValuesForPerson(ResultSet set) throws SQLException {
			...
			result.put("id", set.getString("FK_PERSON"));
			return result;
		}
		// by JavaObjectMapperGenerator
		private Map<String, Object> parseFKValuesForTags(ResultSet set) throws SQLException {
			final Map<String, Object> result = new HashMap<>();
			result.put("id", set.getObject("ID"));
			return result;
		}
	}

	// by JavaObjectMapperGenerator
	public final NamedQuery<test.model.mixedkeys.Address> createNamedQuery(final JavaSession session, String name) {
			if( "selectById".equals(name) ) {
				return new NamedQuery<test.model.mixedkeys.Address>() {
					public final List<test.model.mixedkeys.Address> queryForList(Object... parameters) {
						throw new UnsupportedOperationException("This is a single value query");
					}

					public final test.model.mixedkeys.Address queryForOne(Object... parameters) {
	...
	final static class ProxyData_Address {
		public final test.model.emap.mixedkeys.MPersonMapper.Key person;
		public ProxyData_Address(test.model.emap.mixedkeys.MPersonMapper.Key person) {
			this.person = person;
		}
	}
																											}
