...
public abstract class DynamicSelectQueryImpl<T,O> extends DynamicBaseQuery<T, O> implements DynamicSelectQuery<T,O>, InternalSelectQueryCriteria {
	private final DynamicListDelegate<T,O> listDelegate;
	...
	@Override
	public List<T> list() {
		List<T> l = listDelegate.list(this);
		if( maxRows != -1 ) {
			if( l.size() > maxRows ) {
